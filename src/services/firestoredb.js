...
export async function createPatientDocument(user, additionalData = {}) {
  console.log('🔍 createPatientDocument called with:', {
    user: user?.uid,
    email: user?.email,
    additionalData,
  });
  try {
    ...
    const logger = window.firestoreLogger || console;
    logger.info(
      'Minimal patient document created successfully for:',
      user.email,
      'with provider:',
      authProvider
    );
    ...
    if (error.code === 'permission-denied') {
      errorMessage =
        'Permission denied. This is likely due to Firestore rules. Please check FIRESTORE_RULES_FIX.md for instructions.';
...
export async function updatePatientPersonalInfo(userId, personalInfo) {
  try {
    ...
    if (!rateLimiter.isAllowed(userId, 5, 60000)) {
      throw new Error(
        'Too many update attempts. Please wait 1 minute before trying again.'
      );
    }
...
export async function addMedicalCondition(userId, category, condition) {
  try {
    ...
    if (!rateLimiter.isAllowed(userId, 10, 60000)) {
      throw new Error(
        'Too many condition additions. Please wait 1 minute before trying again.'
      );
    }
...
export async function removeMedicalCondition(userId, category, condition) {
  try {
    ...
    const updatedConditionsInCategory = currentConditions[category].filter(
      c => c !== condition
    );
...
export async function removePatientConsultation(userId, consultationId) {
  try {
    console.log(
      '🔍 removePatientConsultation called for user:',
      userId,
      'consultation:',
      consultationId
    );
    ...
    const currentConsultations =
      patientData?.activity?.consultationHistory || [];
...
export async function addPatientDocument(userId, file, documentName = null) {
  try {
    ...
    if (!rateLimiter.isAllowed(userId, 5, 60000)) {
      throw new Error(
        'Too many document uploads. Please wait 1 minute before trying again.'
      );
    }
    ...
    if (!allowedTypes.includes(file.type)) {
      throw new Error(
        'File type not supported. Please upload JPEG, PNG, GIF, PDF, or text files.'
      );
    }
    ...
    await runTransaction(db, async transaction => {
      ...
      if (patientDoc.exists()) {
        ...
        console.log(
          '📋 Updated documents array in transaction:',
          updatedDocuments
        );
    ...
    console.log(
      '✅ Document added successfully to Firestore using transaction'
    );
...
export async function removePatientDocument(userId, documentId) {
  try {
    console.log(
      '🔍 removePatientDocument called for user:',
      userId,
      'document:',
      documentId
    );
...
export async function getFacilityAppointments(facilityId) {
  try {
    ...
    querySnapshot.forEach(doc => {
      ...
      console.log(
        `Patient ${doc.id} has ${patientAppointments.length} appointments`
      );
      ...
      const facilityAppointments = patientAppointments.filter(appointment => {
        ...
        console.log(
          `Appointment ${appointment.id}: facilityId=${appointment.facilityId}, match=${isMatch}`
        );
        return isMatch;
      });
    ...
    console.log(
      `📊 Summary: ${appointments.length}/${totalAppointments} appointments match facility ${facilityId}`
    );
 ...
 */
export async function updateAppointmentByFacility(
  appointmentId,
  patientId,
  updatedAppointmentData,
  facilityId
        ...
        return {
          ...
          modificationHistory: [
            ...(apt.modificationHistory || []),
            {
              ...
              changes: {
                date:
                  apt.date !== updatedAppointmentData.date
                    ? { from: apt.date, to: updatedAppointmentData.date }
                    : null,
                time:
                  apt.time !== updatedAppointmentData.time
                    ? { from: apt.time, to: updatedAppointmentData.time }
                    : null,
                doctor:
                  apt.doctor !== updatedAppointmentData.doctor
                    ? { from: apt.doctor, to: updatedAppointmentData.doctor }
                    : null,
                status:
                  apt.status !== updatedAppointmentData.status
                    ? { from: apt.status, to: updatedAppointmentData.status }
                    : null,
              },
            },
          ],
...
export async function createAppointmentForPatient(
  ...
  try {
    ...
    if (updatedDocSnap.exists()) {
      ...
      console.log(
        '🔍 Verification - Updated appointments:',
        updatedData.activity?.appointments || []
      );
      console.log(
        '🔍 Verification - Number of appointments:',
        updatedData.activity?.appointments?.length || 0
      );
    }
...
export function listenToFacilityAppointments(facilityId, callback) {
  ...
  const unsubscribe = onSnapshot(patientsRef, querySnapshot => {
    ...
    querySnapshot.forEach(doc => {
      ...
      // Filter appointments for this facility
      const facilityAppointments = patientAppointments.filter(
        appointment => appointment.facilityId === facilityId
      );
...
export async function updateLastLogin(userId) {
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.log(
        '⚠️ Patient document does not exist, skipping last login update'
      );
    }
  } catch (error) {
  ...
  try {
    ...
    return {
      ...
      completedConsultations:
        activity.consultationHistory?.filter(c => c.status === 'completed')
          .length || 0,
      upcomingAppointments:
        activity.appointments?.filter(a => a.status === 'scheduled').length ||
        0,
    };
  } catch (error) {
    ...
    for (const docSnapshot of querySnapshot.docs) {
      ...
      if (!patientData.uniquePatientId) {
        try {
          ...
          console.log(
            `Fixed patient document ${docSnapshot.id} with ID: ${uniquePatientId}`
          );
          fixedCount++;
        } catch (error) {
          console.error(
            `Failed to fix patient document ${docSnapshot.id}:`,
            error
          );
          errorCount++;
        }
...
export async function assignUniquePatientIdToCurrentUser() {
  try {
    ...
    if (patientData.uniquePatientId) {
      console.log(
        'Patient already has unique ID:',
        patientData.uniquePatientId
      );
...
export async function createFacilityDocument(user, additionalData = {}) {
  try {
    ...
    const logger = window.firestoreLogger || console;
    logger.info(
      'Facility documents created successfully for:',
      user.email,
      'with provider:',
      authProvider
    );
    ...
    if (error.code === 'permission-denied') {
      errorMessage =
        'Permission denied. This is likely due to Firestore rules.';
      console.error(
        '🔒 Firestore Permission Denied Error for facility creation:'
      );
      console.error(
        '- Check if user is authenticated:',
        user ? '✅ Yes' : '❌ No'
...
export async function cleanupTestFacilities() {
  try {
    ...
    querySnapshot.forEach(doc => {
      ...
      if (isTestFacility) {
        console.log(
          ...
          data.facilityInfo?.name || data.name
        );
        deleteDoc(doc.ref);
        deletedCount++;
      }
    ...
    console.log(
      `✅ Cleanup complete. Deleted ${deletedCount} test facilities.`
    );
    return deletedCount;
  } catch (error) {
