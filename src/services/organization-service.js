...
class OrganizationService {
      ...
      const orgData = {
        ...
        settings: {
          allowPatientRegistration:
            organizationData.settings?.allowPatientRegistration ?? true,
  ...
  async inviteStaffMember(organizationId, staffData) {
    try {
      ...
      const invitationData = {
        ...
        permissions:
          staffData.permissions || this.getDefaultPermissions(staffData.role),
  ...
  async acceptStaffInvitation(invitationId, password) {
    try {
      ...
      const invitationDoc = await getDoc(
        doc(db, 'staff_invitations', invitationId)
      );
      ...
      return {
        userId: userCredential.user.uid,
        message:
          'Staff account created successfully. Please check your email to verify your account.',
      };
    } catch (error) {
  ...
  validateOrganizationData(data) {
    ...
    if (
      !data.type ||
      !['hospital', 'clinic', 'practice', 'other'].includes(data.type)
    ) {
