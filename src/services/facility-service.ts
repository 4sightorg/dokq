  ...
  async searchFacilities(filters: SearchFilters = {}): Promise<Facility[]> {
    try {
      ...
      querySnapshot.forEach(doc => {
        ...
        console.log(
          '🔍 Facility searchable check:',
          facilityData.isSearchable,
          'for facility:',
          facilityData.name
        );
        ...
        if (facilityData.isSearchable !== false) {
          ...
          console.log(
            '❌ Facility filtered out due to isSearchable:',
            facilityData.name
          );
        }
      });
      ...
      if (filters.type) {
        filteredFacilities = filteredFacilities.filter(
          facility => facility.type === filters.type
        );
      }
      ...
      if (filters.city) {
        filteredFacilities = filteredFacilities.filter(
          facility => facility.city === filters.city
        );
      }
      ...
      if (filters.province) {
        filteredFacilities = filteredFacilities.filter(
          facility => facility.province === filters.province
        );
      }
      ...
      if (filters.services && filters.services.length > 0) {
        filteredFacilities = filteredFacilities.filter(facility =>
          filters.services!.some(service => facility.services.includes(service))
        );
      }
      ...
      console.log(
        `✅ Returning ${filteredFacilities.length} filtered facilities`
      );
    ...
    try {
      ...
      querySnapshot.forEach(doc => {
        ...
        if (facilitySpecialties.length > 0) {
          facilitySpecialties.forEach((specialty: string) =>
