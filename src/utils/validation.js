...
function validateMedicalCondition(condition) {
  ...
  if (sanitized.length > 100) {
    return {
      valid: false,
      error: 'Medical condition must be less than 100 characters',
    };
  }
  ...
  if (!conditionRegex.test(sanitized)) {
    return {
      valid: false,
      error: 'Medical condition contains invalid characters',
    };
  }
  return { valid: true, value: sanitized };
}
function validateCategory(category) {
  ...
  if (!validCategories.includes(sanitized)) {
    return {
      valid: false,
      error: 'Invalid category. Must be: speech, physical, mental, or other',
    };
  }
  return { valid: true, value: sanitized };
}
