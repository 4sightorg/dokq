...
class AuthGuard {
  ...
  async checkCurrentRoute() {
    ...
    try {
      ...
      if (
        protection.requireEmailVerification &&
      ...
      if (protection.roles && protection.roles.length > 0) {
        ...
        if (!userRole) {
          if (
            authService.isAuthenticated() &&
            protection.roles.includes(USER_ROLES.PATIENT)
          ) {
            return true;
          }
        }
      ...
      if (
        authService.isAuthenticated() &&
        currentPath.includes('patientPortal')
      ) {
        return true;
      }
  ...
  applyPageProtections() {
    ...
    this.protectElements('[data-org-admin-only]', [
      USER_ROLES.ORGANIZATION_ADMIN,
    ]);

    this.protectElements('[data-org-member-only]', [
      USER_ROLES.ORGANIZATION_MEMBER,
      USER_ROLES.ORGANIZATION_ADMIN,
    ]);

    this.protectElements('[data-doctor-only]', [
      USER_ROLES.DOCTOR,
      USER_ROLES.ADMIN,
    ]);

    this.protectElements('[data-nurse-only]', [
      USER_ROLES.NURSE,
      USER_ROLES.DOCTOR,
      USER_ROLES.ADMIN,
    ]);

    this.protectElements('[data-staff-only]', [
      USER_ROLES.CLINIC_STAFF,
      USER_ROLES.NURSE,
      USER_ROLES.DOCTOR,
      USER_ROLES.ADMIN,
  ...
  showRoleSpecificContent() {
    ...
    if (userRole) {
      document
        .querySelectorAll(`[data-role="${userRole}"]`)
        .forEach(element => {
          element.style.display = '';
        });

      document
        .querySelectorAll('[data-role="authenticated"]')
        .forEach(element => {
          element.style.display = '';
        });
    } else {
      document
        .querySelectorAll('[data-role="unauthenticated"]')
        .forEach(element => {
          element.style.display = '';
        });
    }
