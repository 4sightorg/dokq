...
const PatientPortal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'general' | 'consultation-history' | 'patient-documents'
  >('general');
  ...
  const [consultationHistoryForm, setConsultationHistoryForm] =
    useState<ConsultationHistoryForm>({
      date: '',
      doctor: '',
      type: 'virtual',
      status: 'completed',
  ...
  const [isLoadingConsultationHistory, setIsLoadingConsultationHistory] =
    useState(false);
  ...
  const [selectedFacilitySpecialty, setSelectedFacilitySpecialty] =
    useState('');
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged(async user => {
      ...
      if (user) {
        ...
        try {
          const localPatientData = localStorage.getItem(
            `patientData_${user.uid}`
          );
          if (localPatientData) {
            const parsedData = JSON.parse(localPatientData);
            console.log(
              '📦 Loaded patient data from localStorage:',
              parsedData
            );
            ...
            if (parsedData?.activity?.appointments) {
              checkForAppointmentModifications(
                parsedData.activity.appointments
              );
            }
        ...
        try {
          console.log(
            '📦 Attempting to load existing patient data from Firestore...'
          );
          ...
          if (existingPatientData) {
            console.log(
              '✅ Found existing patient data in Firestore:',
              existingPatientData
            );
            console.log(
              '📋 Documents in loaded data:',
              existingPatientData?.activity?.documents
            );
            console.log(
              '📋 Number of documents:',
              existingPatientData?.activity?.documents?.length || 0
            );
            ...
            if (existingPatientData?.activity?.appointments) {
              checkForAppointmentModifications(
                existingPatientData.activity.appointments
              );
            }
          ...
          console.warn(
            '⚠️ Failed to load from Firestore, will create new document:',
            error
          );
        }
        ...
        try {
          ...
          const { createPatientDocument } = await import(
          ...
          console.log(
            '✅ Patient document created in Firestore:',
          ...
          if (firestorePatientData?.activity?.appointments) {
            checkForAppointmentModifications(
              firestorePatientData.activity.appointments
            );
          }
        } catch (error) {
          console.warn(
            '⚠️ Failed to create patient document in Firestore, using local data:',
            error
          );
          ...
          try {
            localStorage.setItem(
              `patientData_${user.uid}`,
              JSON.stringify(basicPatientData)
            );
  ...
  useEffect(() => {
    ...
    const setupRealTimeListener = async () => {
      try {
        const { listenToPatientData } = await import(
          '../services/firestoredb.js'
        );
        const unsubscribe = listenToPatientData(
          user.uid,
          updatedPatientData => {
            console.log('🔄 Real-time update received:', updatedPatientData);
            console.log(
              '📋 Appointments in update:',
              updatedPatientData?.activity?.appointments || []
            );
            console.log(
              '📋 Number of appointments:',
              updatedPatientData?.activity?.appointments?.length || 0
            );
  ...
  const handleNavClick = useCallback(
    (section: 'dashboard' | 'profile' | 'facilities') => {
  ...
  const handleTabClick = useCallback(
    (tab: 'general' | 'consultation-history' | 'patient-documents') => {
      setActiveTab(tab);
    },
    []
  );
  ...
  const openModal = useCallback(
    (modalType: string) => {
      ...
      if (modalType === 'editProfile' && patientData?.personalInfo) {
        console.log(
          '📝 Initializing edit profile form with:',
  ...
  const showNotification = useCallback(
    ...
    `;
      ...
      // Auto remove after 5 seconds (no exit animation)
      setTimeout(() => {
  ...
  // Check for appointment modifications and show notifications
  const checkForAppointmentModifications = useCallback(
    (appointments: any[]) => {
      appointments.forEach(appointment => {
        if (
          appointment.updatedBy === 'facility' &&
          appointment.modificationHistory &&
          appointment.modificationHistory.length > 0
        ) {
          const lastModification =
            appointment.modificationHistory[
              appointment.modificationHistory.length - 1
            ];
          ...
          if (now.getTime() - modificationTime.getTime() < 5 * 60 * 1000) {
            ...
            showNotification(
              `Your appointment has been updated by ${appointment.facilityName || 'the healthcare facility'}: ${changes}`,
              'info'
            );
          }
        }
  ...
  const handleBookAppointment = useCallback(async () => {
    ...
    try {
      ...
      const appointmentData = {
        ...
        patientName:
          user.displayName || patientData?.personalInfo?.fullName || 'Patient',
      ...
      try {
        const { getCurrentPatientData } = await import(
          '../services/firestoredb.js'
        );
      ...
      alert(
        'Appointment booked successfully! Check the Dashboard "My Consults" section to see your appointment.'
      );
      closeModal();
    } catch (error: any) {
      ...
      if (error.code === 'permission-denied') {
        errorMessage =
          'You do not have permission to book appointments. Please contact support.';
  ...
  const handleSaveProfile = useCallback(async () => {
    ...
    if (
      ...
      !editProfileForm.fullName.trim()
    ) {
      console.error('❌ Missing required fields');
      alert(
        'Please fill in all required fields (First Name, Last Name, and Full Name)'
      );
      return;
    }
    ...
    try {
      ...
      if (user) {
        try {
          ...
          if (!existingPatientData) {
            console.log(
              '📝 Patient document does not exist, creating it first...'
            );
          ...
          alert(
            'Profile updated locally! Server sync failed. Your changes are saved.'
          );
        }
      } else {
  ...
  const handleAddCondition = useCallback(async () => {
    ...
    try {
      ...
      if (user) {
        try {
          console.log('📦 Attempting Firebase update...');
          const { addMedicalCondition } = await import(
            '../services/firestoredb.js'
          );
          ...
          console.log('✅ Firebase update successful!');
          alert(
            'Medical condition added successfully! Changes saved to server.'
          );
        } catch (firebaseError: any) {
          console.warn('⚠️ Firebase update failed:', firebaseError);
          console.warn(
            'Changes saved locally; server sync will be retried later.'
          );
        }
      }
  ...
  const handleAddConsultationHistory = useCallback(async () => {
    ...
    if (
      !consultationHistoryForm.date ||
      !consultationHistoryForm.doctor.trim()
    ) {
      console.error('❌ Missing required fields');
    ...
    try {
      ...
      if (user) {
        try {
          console.log('📦 Attempting Firebase update...');
          const { addConsultationHistory } = await import(
            '../services/firestoredb.js'
          );
          ...
          console.warn('⚠️ Firebase update failed:', firebaseError);
          alert(
            'Consultation history added locally! Note: Server sync failed.'
          );
        }
      }
  ...
  const handleRemoveCondition = useCallback(
    async (category: string, condition: string) => {
      if (!user) return;

      if (
        !confirm(
          `Are you sure you want to remove "${condition}" from ${category}?`
        )
      ) {
        return;
      }
      ...
      try {
        const { removeMedicalCondition } = await import(
          '../services/firestoredb.js'
        );
        ...
        setPatientData(prev => {
          ...
          const updatedConditions = {
            ...prev.medicalInfo?.conditions,
            [category]:
              prev.medicalInfo?.conditions?.[category]?.filter(
                c => c !== condition
              ) || [],
          };
  ...
  const handleUploadDocument = useCallback(async () => {
    ...
    try {
      ...
      const uploadedDocument = await addPatientDocument(
        user.uid,
        selectedFile,
  ...
  const handleRemoveDocument = useCallback(
    async (documentId: string) => {
      ...
      try {
        ...
        const { removePatientDocument } = await import(
          '../services/firestoredb.js'
        );
        ...
        setPatientData(prev => {
          ...
          const updatedDocuments =
            prev.activity?.documents?.filter(doc => doc.id !== documentId) ||
            [];
  ...
  const handleEditAppointment = useCallback(async () => {
    ...
    // Validate date is not in the past
    const selectedDate = new Date(
      editAppointmentForm.date + 'T' + editAppointmentForm.time
    );
    const now = new Date();
    if (selectedDate <= now) {
      alert('Please select a future date and time for your appointment');
    ...
    try {
      ...
      if (error.code === 'permission-denied') {
        errorMessage =
          'You do not have permission to update appointments. Please contact support.';
      } else if (error.code === 'unavailable') {
        errorMessage =
          'Service temporarily unavailable. Please try again in a few minutes.';
      } else if (error.message) {
  ...
  const handleUpdateSettings = useCallback(async () => {
    ...
    try {
      const { updatePatientSettings } = await import(
        '../services/firestoredb.js'
      );
      ...
      if (
        ...
        console.log(
          'Patient document may not exist yet, updating local state only'
        );
  ...
  const loadConsultationHistory = useCallback(async () => {
    ...
    try {
      const { getConsultationHistory } = await import(
        '../services/firestoredb.js'
      );
  ...
  const handleRemoveConsultation = useCallback(
    async (consultationId: string) => {
      ...
      try {
        console.log(
          '🔍 handleRemoveConsultation called for consultation:',
          consultationId
        );

        // Update local state immediately for better UX
        setConsultationHistory(prev => {
          const updatedConsultations = prev.filter(
  ...
  const getUserDisplayName = useCallback(() => {
    ...
    if (
      ...
      patientData.personalInfo.fullName.trim()
    ) {
      return patientData.personalInfo.fullName;
    }
    if (
  ...
  const getPatientBio = useCallback(() => {
    return patientData?.personalInfo?.bio || 'Welcome to DokQ!';
  ...
  useEffect(() => {
    ...
    if (savedPreferences) {
      try {
        ...
        // Only set activeSection if it's a valid section (not 'help')
        const savedSection = preferences.lastActiveSection;
        if (
          savedSection &&
          ['dashboard', 'profile', 'facilities'].includes(savedSection)
        ) {
  ...
  const loadFacilities = useCallback(async () => {
    ...
    try {
      ...
      const transformedFacilities: FacilityData[] = allFacilities.map(
        facility => {
          console.log(
            '🔍 Processing facility:',
            facility.name,
      ...
      if (transformedFacilities.length === 0) {
        console.log(
          'ℹ️ No facilities found - this is normal if no facilities have signed up yet'
        );
      }
    } catch (error) {
  ...
  const filteredFacilities = useMemo(() => {
    return facilities.filter(facility => {
      ...
      if (
        ...
        facility.facilityInfo.type !== selectedFacilityType
      ) {
        return false;
      }
      ...
      // Filter by city
      if (
        selectedFacilityCity &&
        facility.facilityInfo.city !== selectedFacilityCity
      ) {
        return false;
      }
      ...
      // Filter by specialty
      if (
        selectedFacilitySpecialty &&
        !facility.specialties.includes(selectedFacilitySpecialty)
      ) {
        return false;
      }
      ...
      >
        ...
        <nav className='sidebar-nav'>
          <ul className='nav-items' role='menubar'>
            ...
            <li
              className={`nav-item ${activeSection === 'facilities' ? 'active' : ''}`}
              role='none'
            >
              <button
                className='nav-link'
                ...
                role='menuitem'
                aria-current={
                  activeSection === 'facilities' ? 'page' : undefined
                }
          ...
          {activeSection === 'dashboard' && (
            <section className='content-section active'>
              ...
              <div className='greeting'>
                <h1>
                  Hi, <span>{getUserDisplayName()}!</span>
                </h1>
              ...
              <div className='stats-grid'>
                <div className='stat-card'>
                  ...
                  <div className='stat-info'>
                    <h3>
                      {patientData?.activity?.appointments?.filter(
                ...
                <div className='stat-card'>
                  ...
                  <div className='stat-info'>
                    <h3>
                      {patientData?.profileComplete ? 'Complete' : 'Incomplete'}
                    </h3>
                    <p>Profile Status</p>
                  </div>
                </div>
              ...
              <div className='quick-actions'>
                ...
                <div className='action-buttons'>
                  <button
                    ...
                    onClick={() => {
                      console.log(
                        '🔘 Book Appointment button clicked in Quick Actions!'
                      );
              ...
              <div className='dashboard-section'>
                ...
                <div className='appointments-list'>
                  ...
                  patientData.activity.appointments.length > 0 ? (
                    patientData.activity.appointments
                      .sort(
                        (a, b) =>
                      ...
                      .map(appointment => (
                        <div
                          key={appointment.id}
                          className={`appointment-card ${appointment.updatedBy === 'facility' ? 'has-modifications' : ''}`}
                        >
                          ...
                          <div className='appointment-info'>
                            ...
                            <div className='appointment-facility'>
                              {appointment.facilityName || 'Facility TBD'}
                            </div>
                  ...
                  <div className='tab-content active'>
                      ...
                      <div className='profile-info'>
                        ...
                        <div className='profile-details'>
                          <h3 className='profile-name'>
                            {getUserDisplayName()}{' '}
                            <span className='gender-tag'>(Patient)</span>
                          </h3>
                    ...
                    <div className='info-section'>
                      <div className='section-header'>
                        ...
                        <button
                          ...
                          onClick={() => {
                            console.log(
                              '🔘 Edit Personal Information button clicked'
                            );
                      ...
                      !patientData?.personalInfo?.lastName ? (
                        ...
                        >
                          ...
                          ></i>
                          <strong>Profile Incomplete:</strong> Please complete
                          your profile information for better healthcare
                          services.
                        </div>
                      ) : null}
                    </div>
                    ...
                    <div className='info-section'>
                      ...
                      <div className='diseases-section'>
                        {patientData?.medicalInfo?.conditions &&
                        Object.keys(patientData.medicalInfo.conditions).length >
                          0 ? (
                          Object.entries(
                            patientData.medicalInfo.conditions
                              ...
                              <div className='disease-tags'>
                                {conditions.map((condition, index) => (
                                  <span
                                    ...
                                    onClick={() =>
                                      handleRemoveCondition(category, condition)
                                    }
                      ...
                      {isLoadingConsultationHistory ? (
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className='appointment-time-slot'>
                                    {formatTime(consultation.time || '00:00')} -{' '}
                                    {dateGroup}
                                  </div>
                                  <div className='appointment-details'>
                                    ...
                                    <span
                                      className={`appointment-status status-${consultation.status}`}
                                    >
                                      {consultation.status}
                                    </span>
                                  </div>
                        ...
                        <div className='no-consultations'>
                          <div className='empty-state'>
                            ...
                            <p>
                              You haven't added any consultation history yet.
                              Add your first consultation to get started.
                            </p>
                            <button
                              className='btn btn-primary'
                              onClick={() =>
                                openModal('addConsultationHistory')
                              }
                            >
                              <i className='fas fa-plus'></i>
                ...
                {activeTab === 'patient-documents' && (
                  <div className='tab-content active'>
                    ...
                    <div className='documents-grid'>
                      {patientData?.activity?.documents &&
                      patientData.activity.documents.length > 0 ? (
                        ...
                        <div className='no-documents'>
                          <div className='empty-state'>
                            ...
                            <button
                              ...
                              onClick={() => {
                                console.log(
                                  '🔘 Upload Document button clicked (empty state)!'
                                );
                                openModal('uploadDocument');
                              }}
                            >
          ...
          {activeSection === 'facilities' && (
            <section className='content-section active'>
              <div className='section-header'>
                ...
                <p>
                  Find and book appointments with healthcare providers near you
                </p>
              </div>
                ...
                <div className='search-filters'>
                  ...
                  <div className='filter-row'>
                    ...
                    <select
                      ...
                      onChange={e =>
                        setSelectedFacilitySpecialty(e.target.value)
                      }
                    >
              ...
              <div className='facilities-grid'>
                {isLoadingFacilities ? (
                    ...
                    <div key={facility.uid} className='facility-card'>
                      <div className='facility-header'>
                        <div className='facility-icon'>
                          <i
                            className={`fas fa-${facility.facilityInfo.type === 'Hospital' ? 'hospital' : 'stethoscope'}`}
                          ></i>
                        </div>
                        <div className='facility-info'>
                          <h3>{facility.facilityInfo.name}</h3>
                          <p className='facility-type'>
                            {facility.facilityInfo.type}
                          </p>
                          <p className='facility-location'>
                            <i className='fas fa-map-marker-alt'></i>
                            {facility.facilityInfo.city &&
                            facility.facilityInfo.province
                        ...
                        <div className='facility-hours'>
                          ...
                          {facility.operatingHours && (
                            <>
                              <p>
                                Monday - Friday:{' '}
                                {facility.operatingHours.monday.open} -{' '}
                                {facility.operatingHours.monday.close}
                              </p>
                              <p>
                                Saturday:{' '}
                                {facility.operatingHours.saturday.open} -{' '}
                                {facility.operatingHours.saturday.close}
                              </p>
                              <p>
                                Sunday:{' '}
                                {facility.operatingHours.sunday.closed
                                  ? 'Closed'
                                  : `${facility.operatingHours.sunday.open} - ${facility.operatingHours.sunday.close}`}
                              </p>
                            </>
                          )}
                        </div>
                  ...
                  <div className='empty-state'>
                    ...
                    <p>
                      There are currently no healthcare facilities registered in
                      the system.
                    </p>
                    <p>
                      This is normal if no facilities have signed up yet.
                      Facilities will appear here once they register.
                    </p>
                    <div
                      style={{
                        marginTop: '1rem',
                    ...
                    >
                      ...
                      >
                        ...
                        ></i>
                        <strong>For Healthcare Facilities:</strong> Sign up
                        through the Partner Registration to appear in this list.
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className='modal' style={{ display: 'block' }}>
          <div className='modal-content'>
            ...
            <div className='modal-body'>
              ...
              >
                <div className='form-group'>
                  <label htmlFor='appointmentFacility'>
                    Healthcare Facility *
                  </label>
                  <select
            ...
            <div className='modal-body'>
              <form
                ...
                onSubmit={e => {
                  e.preventDefault();
                  console.log(
                    'Add condition form submitted, calling handleAddCondition...'
                  );
                  handleAddCondition();
                }}
              >
      ...
      {showModal === 'addConsultationHistory' && (
        <div className='modal' style={{ display: 'block' }}>
          <div className='modal-content'>
            ...
            <div className='modal-body'>
              <form
                id='addConsultationHistoryForm'
                onSubmit={e => {
                  e.preventDefault();
                  console.log(
                    'Add consultation history form submitted, calling handleAddConsultationHistory...'
                  );
              ...
              >
                <div className='form-group'>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setConsultationHistoryForm(prev => ({
                        ...prev,
                        date: e.target.value,
                      }))
                    }
            ...
            <div className='modal-footer'>
              ...
              <button
                ...
                disabled={
                  ...
                  !consultationHistoryForm.date ||
                  !consultationHistoryForm.doctor
                }
              >
          ...
          <div className='modal-content document-viewer-modal'>
            ...
            <div className='modal-body document-viewer-body'>
              <div className='document-info'>
                <p>
                  <strong>File Name:</strong> {viewingDocument.originalName}
                </p>
                <p>
                  <strong>Size:</strong>{' '}
                ...
                <p>
                  <strong>Type:</strong> {viewingDocument.type}
                </p>
                <p>
            ...
            <div className='modal-body'>
              <form
                ...
                onSubmit={e => {
                  e.preventDefault();
                  console.log(
                    'Edit appointment form submitted, calling handleEditAppointment...'
                  );
                  handleEditAppointment();
                }}
              >
                <div className='form-group'>
                  <label htmlFor='editAppointmentFacility'>
                    Healthcare Facility *
                  </label>
                  <select
                    ...
                    onChange={e => {
                      ...
                      const facilityName =
                        e.target.options[e.target.selectedIndex].text;
                      console.log('Selected facility for edit:', {
                        facilityId,
                        facilityName,
                      });
                ...
                <div className='form-group'>
                  <label htmlFor='editAppointmentType'>
                    Appointment Type *
                  </label>
                  <select
                ...
                <div className='form-group'>
                  <label htmlFor='editAppointmentDoctor'>
                    Doctor (Optional)
                  </label>
                  <input
                    type='text'
            ...
            <div className='modal-body'>
              <div className='facility-details-content'>
                    ...
                    <div className='info-item'>
                      ...
                      <span>
                        {selectedFacility.facilityInfo.website ? (
                          <a
                            href={selectedFacility.facilityInfo.website}
                            target='_blank'
                            rel='noopener noreferrer'
                          >
                            {selectedFacility.facilityInfo.website}
                ...
                <div className='facility-section'>
                  ...
                  <p>
                    {selectedFacility.facilityInfo.city},{' '}
                    {selectedFacility.facilityInfo.province}{' '}
                    {selectedFacility.facilityInfo.postalCode}
                  </p>
                ...
                {selectedFacility.specialties &&
                  selectedFacility.specialties.length > 0 && (
                    <div className='facility-section'>
                      <h4>
                        <i className='fas fa-stethoscope'></i> Specialties
                      </h4>
                      <div className='tags-container'>
                        {selectedFacility.specialties.map(
                          (specialty, index) => (
                            <span key={index} className='tag specialty-tag'>
                              {specialty}
                            </span>
                          )
                        )}
                ...
                {selectedFacility.services &&
                  selectedFacility.services.length > 0 && (
                    <div className='facility-section'>
                      <h4>
                        <i className='fas fa-medical-kit'></i> Services
                      </h4>
                      <div className='tags-container'>
                ...
                <div className='facility-section'>
                  ...
                  <div className='hours-grid'>
                    <div className='hour-item'>
                      ...
                      <span className='time'>
                        {selectedFacility.operatingHours.monday.closed
                          ? 'Closed'
                    ...
                    <div className='hour-item'>
                      ...
                      <span className='time'>
                        {selectedFacility.operatingHours.saturday.closed
                          ? 'Closed'
                    ...
                    <div className='hour-item'>
                      ...
                      <span className='time'>
                        {selectedFacility.operatingHours.sunday.closed
                          ? 'Closed'
                  ...
                  <div className='staff-grid'>
                    <div className='staff-item'>
                      ...
                      <span className='staff-count'>
                        {selectedFacility.staff.totalStaff}
                      </span>
                    </div>
                    <div className='staff-item'>
                    ...
                    <div className='staff-item'>
                      ...
                      <span className='staff-count'>
                        {selectedFacility.staff.nurses}
                      </span>
                    </div>
                    <div className='staff-item'>
                      <span className='staff-type'>Support Staff:</span>
                      <span className='staff-count'>
                        {selectedFacility.staff.supportStaff}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className='facility-section'>
                  <h4>
                    <i className='fas fa-certificate'></i> Additional
                    Information
                  </h4>
                  <div className='additional-info'>
                    <div className='info-item'>
                      ...
                      <span>
                        {selectedFacility.licenseNumber || 'Not provided'}
                      </span>
                    </div>

                    {selectedFacility.accreditation &&
                      selectedFacility.accreditation.length > 0 && (
                        <div className='info-item'>
                          <label>Accreditations:</label>
                          <div className='tags-container'>
                            {selectedFacility.accreditation.map(
                              (acc, index) => (
                                <span
                                  key={index}
                                  className='tag accreditation-tag'
                                >
                                  {acc}
                                </span>
                              )
                    ...
                    {selectedFacility.insuranceAccepted &&
                      selectedFacility.insuranceAccepted.length > 0 && (
                        <div className='info-item'>
                          <label>Insurance Accepted:</label>
                          <div className='tags-container'>
                            {selectedFacility.insuranceAccepted.map(
                              (insurance, index) => (
                                <span key={index} className='tag insurance-tag'>
                                  {insurance}
                                </span>
                              )
                            )}
                    ...
                    {selectedFacility.languages &&
                      selectedFacility.languages.length > 0 && (
                        <div className='info-item'>
                          <label>Languages:</label>
                          <div className='tags-container'>
                            {selectedFacility.languages.map(
                              (language, index) => (
