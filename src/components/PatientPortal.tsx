...
const PatientPortal: React.FC = () => {
  ...
  const [activeSection, setActiveSection] = useState<'dashboard' | 'profile' | 'facilities'>('dashboard')
  ...
  const handleNavClick = useCallback((section: 'dashboard' | 'profile' | 'facilities') => {
    ...
    try {
      ...
      if (user) {
        try {
          ...
          console.warn('⚠️ Firebase update failed:', firebaseError)
          console.warn('Changes saved locally; server sync will be retried later.')
        }
      }
  ...
  useEffect(() => {
    ...
    if (savedPreferences) {
      try {
        ...
        if (savedSection && ['dashboard', 'profile', 'facilities'].includes(savedSection)) {
