...
import { signInWithPopup, signInWithRedirect, getRedirectResult, GoogleAuthProvider, signInWithEmailAndPassword, sendPasswordResetEmail } from 'firebase/auth'
...
const PatientSignIn: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    // Handle possible Google redirect sign-in result
    (async () => {
      try {
        ...
        if (redirectResult?.user) {
          ...
          setTimeout(() => navigate('/patient-portal'), 1200)
        }
      } catch (err) {
        // no-op: redirect flow not used or failed silently
      }
    })()
    ...
    try {
      ...
      switch (error.code) {
        ...
        case 'auth/cancelled-popup-request':
          ...
          break
        default:
          if (error?.code === 'auth/network-request-failed' || error?.code === 'auth/popup-blocked' || error?.code === 'auth/cancelled-popup-request') {
            // Fallback to redirect-based sign-in which is more reliable with cookie/csp restrictions
            try {
              const provider = new GoogleAuthProvider()
              provider.addScope('email')
              provider.addScope('profile')
              ...
              errorMsg = redirectErr?.message || errorMsg
            }
  ...
  return (
    <div className="signup-container centered-container" role="main" aria-label="Patient Sign In">
      ...
      <div className="centered-section">
        ...
        <div className="signup-content">
          <div className="signup-header">
            <h1>Welcome Back</h1>
            <p>New to DokQ? <a href="/patient-sign-up" className="login-link">Sign up</a></p>
          </div>
